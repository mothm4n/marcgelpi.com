...
acceptance_browser_assert_eval \
  "About presents Marc's exact CV values and five described CliftonStrengths" \
  "(() => { const heading = document.querySelector('#principles-title'); const values = Array.from(document.querySelectorAll('[data-about-values] li')).map(item => item.textContent.trim()).join('|'); const strengths = Array.from(document.querySelectorAll('[data-about-strengths] li')); return heading?.tagName === 'H2' && heading.textContent.trim() === 'Guiding how I work and create impact:' && values === 'Always people first|Transparency & trust by default|Always believe that we can do things differently|That everything you do adds value|Actions speak louder than words|Always align what I think&feel, what I say and what I do' && strengths.length === 5 && strengths.map(item => item.querySelector('h3')?.textContent.trim()).join('|') === 'Strategic thinking|Analytical|Ideation|Relationship building|Responsibility' && strengths.every(item => (item.querySelector('p')?.textContent.trim().length ?? 0) > 25); })()" \
  "true"
