...
acceptance_browser_assert_eval \
  ...
  "(async () => { const paths = $public_paths_json; const articlePaths = new Set($article_paths_json); const articlePath = '/writing/life-isnt-always-a-river/'; const articleSearchDescription = 'A changed product decision is not automatically a bad one. See how visible reasoning helps teams distinguish learning from chaos.'; const articleSocialDescription = 'Product decisions change. That doesn’t always mean they were wrong.'; const pages = await Promise.all(paths.map(async path => { const response = await fetch(path); const html = await response.text(); const doc = new DOMParser().parseFromString(html, 'text/html'); const meta = key => Array.from(doc.querySelectorAll('meta')).find(node => node.getAttribute('name') === key || node.getAttribute('property') === key)?.content; return { path, ok: response.ok, lang: doc.documentElement.lang, title: doc.title, description: meta('description'), canonical: doc.querySelector('link[rel=\"canonical\"]')?.href, ogTitle: meta('og:title'), ogDescription: meta('og:description'), ogUrl: meta('og:url'), ogType: meta('og:type'), ogLocale: meta('og:locale'), ogImage: meta('og:image'), twitterCard: meta('twitter:card'), twitterDescription: meta('twitter:description') }; })); const uniqueTitles = new Set(pages.map(page => page.title)).size === pages.length; const uniqueDescriptions = new Set(pages.map(page => page.description)).size === pages.length; const accurate = pages.every(page => { const expectedType = page.path === '/about/' ? 'profile' : articlePaths.has(page.path) ? 'article' : 'website'; const expectedSocialTitle = page.path === '/' ? 'Marc Gelpí' : page.title; const expectedSocialDescription = page.path === '/' ? 'People-first organizational effectiveness and ways of working.' : page.path === articlePath ? articleSocialDescription : page.description; const expectedSearchDescription = page.path === articlePath ? articleSearchDescription : page.description; return page.ok && page.lang === 'en' && page.title && page.description === expectedSearchDescription && page.canonical === 'https://marcgelpi.com' + page.path && page.ogTitle === expectedSocialTitle && page.ogDescription === expectedSocialDescription && page.twitterDescription === expectedSocialDescription && page.ogUrl === page.canonical && page.ogType === expectedType && page.ogLocale === 'en_GB' && page.ogImage?.startsWith('https://marcgelpi.com/') && page.twitterCard === 'summary_large_image'; }); return uniqueTitles && uniqueDescriptions && accurate; })()" \
  "true"
